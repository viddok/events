...
function add_events_settings_meta_box() {
	add_meta_box(
	        'event_fields',
            __( 'Настройки события', 'lang' ),
            'events_settings_meta_box',
            'events',
            'normal',
            'high'
    );
...
function events_settings_meta_box( $post ) {
	?>
    <p><?php _e( 'Статус события', 'lang' ); ?>: <?php $mark_v = get_post_meta( $post->ID, 'status', true ); ?>
		<?php $mark_v = ( '' == $mark_v ) ? 'open' : $mark_v; ?>
        <label><input type="radio" name="event[status]"
                      value="open" <?php checked( $mark_v, 'open' ); ?> /> <?php _e( 'Открытое', 'lang' ); ?></label>
        <label><input type="radio" name="event[status]"
                      value="closed" <?php checked( $mark_v, 'closed' ); ?> /> <?php _e( 'По приглашению', 'lang' ); ?>
        </label>
    </p>
    <p>
        <label>
			...
			<?php if ( '' === get_post_meta( $post->ID, 'date', true ) ) {
				...
				$date_val = date( 'd-m-Y', get_post_meta( $post->ID, 'date', true ) );
			} ?>
            <input type="text" name="event[date]" value="<?php echo $date_val; ?>" style="width:50%"
                   placeholder="18-04-2018"/>
        </label>
    </p>
